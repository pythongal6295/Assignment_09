...
# KKauffman, 2020-Sept-09 Added to select_cd and add_track methods
#------------------------------------------#
...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        """
        # TO Done add code as required 
        try:
            cd_idx = int(cd_idx)
        except ValueError:
             print('/nCD ID must be an integer.')
        for row in table:
            if row.cd_id == cd_idx:
                return row
            raise Exception('\nCD ID is not part of the list of CDs. Try entering a different ID.')
